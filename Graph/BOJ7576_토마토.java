...
public class BOJ7576_토마토 {
    ...
    /**
     * <a href= "https://www.acmicpc.net/problem/7576"> 링크 </a>
     * <h1>문제</h1>
     * <p>
     * 철수의 토마토 농장에서는 토마토를 보관하는 큰 창고를 가지고 있다. 토마토는 아래의 그림과 같이 격자모양 상자의 칸에 하나씩 넣은 다음, 상자들을 수직으로 쌓아 올려서 창고에 보관한다. <br>
     *
     * <img style="width=20px; height=20px;" src="https://u.acmicpc.net/de29c64f-dee7-4fe0-afa9-afd6fc4aad3a/Screen%20Shot%202021-06-22%20at%202.41.22%20PM.png"></img> <br>
     * <p>
    ...
    public static void bfs() {
        ...
        while (!queue.isEmpty()) { // Queue가 빌 때까지 반복
            ...
            for (int i = 0; i<4; i++) {
                ...
                if (next[0] >= 0 && next[1] >= 0 //모든 값들이 0보다 큰지
                        && next[0] < N && next[1] < M //범위 내에 있는지
                ) {
                    if( storage[next[0]][next[1]] != -1 && !visited[next[0]][next[1]] ) { //갈 수 있는 길인지 & 방문 했던 길인지 검사
                        queue.add(next);
                        storage[next[0]][next[1]] = storage[current[0]][current[1]] +1 ; // 지나간 길은 현재의 +1
                        visited[next[0]][next[1]] = true; //방문 기록
                        if (maxValue < storage[next[0]][next[1]]) {
                            maxValue = storage[next[0]][next[1]];
                        }
                    }
                }
            }
